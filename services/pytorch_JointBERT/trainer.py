...
from opennlu.services.pytorch_JointBERT.utils import MODEL_CLASSES, set_seed, compute_metrics, get_intent_labels, get_slot_labels
...
class Trainer(object):
    ...
    def evaluate(self, mode):
        # We use test dataset because semeval doesn't have valid dataset
        if mode == 'test':
        ...
        # Intent result
        softmax = [max(np.exp(pred) / np.sum(np.exp(pred), axis=0)) for pred in intent_preds]
        ...
        # Process for display on UI
        intent_preds = [self.intent_label_lst[i] for i in intent_preds]
        out_intent_label_ids = [self.intent_label_lst[i] for i in out_intent_label_ids]
        slot_preds_list = [' '.join(row) for row in slot_preds_list]
        out_slot_label_list = [' '.join(row) for row in out_slot_label_list]

        return [results, intent_preds, out_intent_label_ids, slot_preds_list, out_slot_label_list, softmax]
    ...
    def predict(self, texts, tokenizer):
        ...
        # Intent prediction
        intent_preds = intent_logits.detach().cpu().numpy()
        softmax = max(np.exp(intent_preds[0]) / np.sum(np.exp(intent_preds[0]), axis=0))

        intent_preds = np.argmax(intent_preds, axis=1)

        intent_list = [] # array will only at most be size 1
        ...
        for intent, slots in zip(intent_list, slot_preds_list): # 1 iteration only
            intent_pred = intent
            slots_pred = ' '.join(slots)

        return [intent_pred, slots_pred, softmax]
